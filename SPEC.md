...
| 命令 | 描述 |
|------|------|
| `/bind <path>` | 将当前频道绑定到工作目录，创建 tmux 窗口并启动 coding CLI |
| `/bind-window <window>` | 将当前频道绑定到已有的 tmux 窗口 |
| `/unbind` | 解除当前频道的绑定 |
| `/screenshot` | **手动截屏**：截取当前绑定窗口的终端画面，发送 PNG 图片 |
| `/send-keys <keys>` | 向绑定窗口发送特殊按键（Escape, C-c, Up, Down 等） |
| `/windows` | 列出所有 tmux 窗口及其绑定状态 |
| `/status` | 查看当前绑定窗口的状态 |
| `/new` | 在当前绑定目录创建新的 coding CLI 会话 |
| `/resume <session_id>` | 恢复一个已有的 Claude Code 会话 |
| `/compact` | 向 Claude Code 发送 /compact 命令 |

### 消息处理流程

```
用户在 Discord #claude-main 频道发送: "帮我修复这个 bug"
    │
    ▼
Discord Adapter (discord.py) 收到 on_message 事件
    │
    ▼
Core Engine 查找 #claude-main 的 SessionBinding → tmux window "main"
    │
    ▼
TmuxController.send_text("main", "帮我修复这个 bug", enter=True)
    │ (通过 libtmux 的 pane.send_keys，参考 ccbot 的特殊处理)
    ▼
OutputMonitor 检测到 JSONL 新输出 / pane 内容变化
    │
    ▼
格式化为 Discord 消息（Markdown 代码块）
    │
    ▼
Discord Adapter.send_message() → 分块处理 2000 字符限制
    │
    ▼ （如果 terminal_parser 检测到交互式 UI）
InteractiveUIBridge.to_buttons() → Discord 按钮组件
    │
    ▼
用户点击 "Allow" 按钮 → send_keys("y", enter=True) → tmux pane
```
...
Discord Adapter 收到 interaction
    │
    ▼
ScreenshotEngine.capture(tmux_ctrl, window_id)
    │
    ├── tmux capture-pane -e -p -t {window_id}  (带 ANSI 颜色)
    │
    ├── 解析 ANSI 转义序列
    │
    ├── Pillow 逐字符渲染（3 级字体级联）
    │
    └── 返回 PNG bytes
    │
    ▼
Discord Adapter 以图片附件形式发送到频道
```
