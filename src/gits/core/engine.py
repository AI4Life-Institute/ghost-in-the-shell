...
class Engine:
    ...
    def __init__(self, settings: Settings):
        ...
        # window / Discord thread / registry write / binding all stay in the
        # engine (handle_bos_*). Dormant until builder_os_root is configured and
        # a Discord id is mapped in builder_humans.json — every verb is
        # fail-closed on identity first (§11.4).
        self.builder_launcher = BuilderLauncher(
            builder_os_cmd=settings.builder_os_cmd,
            builder_os_root=settings.builder_os_root,
        )
    ...
    # ------------------------------------------------------------------
    # Builder-OS /bos commands (G6, 0002 §5.5/§5.7) — the launch + lifecycle
    # entry points. Every verb is fail-closed on identity FIRST (§11.4, AC4):
    # an unmapped Discord id is refused before any subprocess or state write.
    # Ghost never writes builder-os runtime-state (§5.5) — all lifecycle
    # mutations go through the builder-os CLI via ``self.builder_launcher``.
    # ------------------------------------------------------------------

    async def handle_bos_start(
        self, channel_id: str, user_id: str, issue: int,
        repo: str | None = None, interaction: Any = None,
    ) -> None:
        """`/bos start issue:<n> [repo:<alias>]` — admit → thread + window + register.

        Fail-before-side-effects (R11): the registry is checked before anything
        observable happens. An already-admitted ticket with a live window is a
        no-op pointer; with a dead window it refuses toward ``/bos resume`` — ghost
        never auto-double-starts. Otherwise: ``ticket admit`` (idempotent) yields
        the LaunchSpec, ghost creates the thread + tmux window + registry entry +
        binding, arms session capture, and posts a **plain ack** — the
        ``driver.started`` card arrives later through the monitor→renderer event
        path (never fabricated synchronously).
        """
        actor = await self._bos_actor_or_refuse(channel_id, user_id, interaction)
        if actor is None:
            return
    ...
    async def _handle_bos_start_locked(
        ...
        if self._adapter is not None:
            try:
                thread_id = await self._adapter.create_thread(
                    channel_id, self._bos_thread_title(spec, issue))
            except Exception:
                logger.warning(
                    "bos start: thread create failed for %s — using channel",
                    uid, exc_info=True)
                thread_id = channel_id

        window_name = self._bos_window_name(uid)
        cmd = self._bos_launch_command(spec)
        win = await self.tmux.create_window(
            name=window_name, cwd=spec.work_dir, command=cmd)
        ...
        await self.session_mgr.bind(
            ...
            coding_cli=spec.cli,
            parent_channel_id=channel_id if thread_id != channel_id else None,
            builder_ticket_uid=uid,
        )
        self.monitor.start_polling(thread_id, win.window_id)
        # (G5) session capture reuses the ONE session_map: jsonl_monitor stamps
        # binding.cli_session_id from the SessionStart hook; we wait for it, then
        # record-session. No builder-parallel map (PM ruling #2).
        self._spawn_session_capture(uid, thread_id, win.window_id)

        await self._bos_ack(
            channel_id, interaction,
            f"✅ `/bos start` — admitted **{spec.display_id or uid}** "
    ...
    async def _bos_start_existing(
        ...
        """Idempotent-start disposition for an already-registered ticket (R11)."""
        binding = self._binding_for_ticket(uid)
        alive = bool(binding) and await self.tmux.window_exists(binding.window_id)
        if alive:
            await self._bos_ack(
                channel_id, interaction,
                f"ℹ️ **{uid}** is already running (<#{binding.channel_id}>, "
                f"tmux `{binding.window_id}`). Nothing to do.",
            )
        else:
            await self._bos_refuse(
                channel_id, interaction, "⛔ Ticket already admitted",
                f"**{uid}** is admitted but its driver window is not alive. "
                f"Use `/bos resume ticket:{uid}` (add takeover if a zombie may "
                f"linger) — ghost never auto-double-starts.",
            )

    async def handle_bos_status(
        self, channel_id: str, user_id: str,
        ticket: str | None = None, interaction: Any = None,
    ) -> None:
        """`/bos status [ticket]` — render the read-only projection (never guess)."""
        actor = await self._bos_actor_or_refuse(channel_id, user_id, interaction)
        if actor is None:
            return
        uid = ticket or self._bos_ticket_for_channel(channel_id)
        if not uid:
            await self._bos_refuse(
                channel_id, interaction, "❓ No ticket",
                "Run `/bos status` in a bound ticket thread, or pass "
                "`ticket:<uid>`.")
            return
        try:
            st = await self.builder_launcher.status(uid)
        except BuilderLaunchError as exc:
            await self._bos_refuse(
                channel_id, interaction, "⚠️ /bos status failed", str(exc))
            return
        await self._bos_ack(channel_id, interaction, self._format_bos_status(uid, st))

    async def handle_bos_respond(
        self, channel_id: str, user_id: str, choice: str, interaction: Any = None,
    ) -> None:
        """`/bos respond choice:<id>` — answer the thread's open decision.

        Reuses T7's authenticated path (:meth:`BuilderResponseAdapter.respond`),
        which fail-closes on identity and passes the capability token through. The
        actor is also gated here so an unmapped user is refused with no side
        effect on *this* verb too (AC4).
        """
        actor = await self._bos_actor_or_refuse(channel_id, user_id, interaction)
        if actor is None:
            return
        uid = self._bos_ticket_for_channel(channel_id)
        if not uid:
            await self._bos_refuse(
                channel_id, interaction, "❓ Not a builder ticket thread",
                "`/bos respond` must run in the ticket's bound thread.")
            return
        decision_id = self.builder_renderer.first_open_decision(uid)
        if not decision_id:
            await self._bos_refuse(
    ...
    async def handle_bos_resume(
        ...
        takeover: bool = False, interaction: Any = None,
    ) -> None:
        """`/bos resume [ticket] [--takeover]` — fence the old window, relaunch.

        builder-os refuses a resume unless the prior window is confirmed dead, so
        ghost **kills it first** (positive confirmation), then ``ticket resume
        [--takeover] --fenced-confirmed``. ``--takeover`` bumps the epoch so a
        surviving zombie CLI is fenced out of every mutating verb (D5). Relaunches
        from the returned LaunchSpec (``--resume`` iff the transcript survived —
        ghost is a dumb executor of ``cli_args``) and re-arms session capture.
        """
        actor = await self._bos_actor_or_refuse(channel_id, user_id, interaction)
        if actor is None:
            return
        ...
        if not uid:
            await self._bos_refuse(
                channel_id, interaction, "❓ No ticket",
                "Run `/bos resume` in a bound ticket thread, or pass "
                "`ticket:<uid>`.")
            return
        ...
        try:
            spec = await self.builder_launcher.resume(
                uid, takeover=takeover, fenced_confirmed=True)
        except BuilderLaunchError as exc:
            await self._bos_refuse(
                channel_id, interaction, "⚠️ /bos resume failed", str(exc))
            return

        target_channel = binding.channel_id if binding is not None else channel_id
        window_name = binding.window_name if binding is not None else self._bos_window_name(uid)
        cmd = self._bos_launch_command(spec)
        win = await self.tmux.create_window(
            name=window_name, cwd=spec.work_dir, command=cmd)

        if binding is not None:
            await self.session_mgr.update_window_id(target_channel, win.window_id)
        else:
            await self.session_mgr.bind(
                platform="discord",
                channel_id=target_channel,
        ...
        self.monitor.start_polling(target_channel, win.window_id)
        self._spawn_session_capture(uid, target_channel, win.window_id)

        await self._bos_ack(
            channel_id, interaction,
            f"🔁 `/bos resume{' --takeover' if takeover else ''}` — relaunched "
            f"**{uid}** (epoch {spec.epoch}) in <#{target_channel}>.",
        )

    async def handle_bos_forward(
        self, channel_id: str, user_id: str, text: str, interaction: Any = None,
    ) -> None:
        """`/bos forward <text>` — bypass suppression, audit, inject to the pane.

        The command layer is fail-closed on identity (AC4) even though the
        underlying forced-forward *mechanic* audits the raw id (T7): an unmapped
        user is refused here with no injection.
        """
        actor = await self._bos_actor_or_refuse(channel_id, user_id, interaction)
        if actor is None:
            return
        forwarded = await self.forced_forward(channel_id, user_id, text)
        if not forwarded:
            await self._bos_refuse(
                channel_id, interaction, "❓ Not a builder ticket thread",
                "`/bos forward` must run in a thread bound to an open builder "
                "ticket.")
            return
        await self._bos_ack(
            channel_id, interaction,
            "📨 Forwarded to the driver pane (bypassed suppression; audited).")

    async def handle_bos_rerun_review(
        self, channel_id: str, user_id: str,
        ticket: str | None = None, interaction: Any = None,
    ) -> None:
        """`/bos rerun-review [ticket]` — re-review the candidate, new round.

        Epoch-fenced: ``session``/``epoch`` are read fresh from ``driver status``
        at invocation time, never cached in the registry (PM ruling #1, §5.5).
        """
        actor = await self._bos_actor_or_refuse(channel_id, user_id, interaction)
        if actor is None:
            return
        ...
        if not uid:
            await self._bos_refuse(
                channel_id, interaction, "❓ No ticket",
                "Run `/bos rerun-review` in a bound ticket thread, or pass "
                "`ticket:<uid>`.")
            return
        try:
            st = await self.builder_launcher.status(uid)
            ...
            await self._bos_refuse(
                channel_id, interaction, "⚠️ /bos rerun-review failed", str(exc))
            return
        session = st.get("driver_session_id")
        epoch = st.get("epoch")
        if not session or epoch is None:
            await self._bos_refuse(
                channel_id, interaction, "⛔ No live session",
                f"**{uid}** has no admitted driver session/epoch to fence "
                f"against (state `{st.get('state')}`).")
            return
        try:
            await self.builder_launcher.rerun_review(uid, session, int(epoch))
        except BuilderLaunchError as exc:
            await self._bos_refuse(
                channel_id, interaction, "⚠️ /bos rerun-review failed", str(exc))
            return
        await self._bos_ack(
            channel_id, interaction,
            f"🔁 `/bos rerun-review` — new review round requested for **{uid}**.")

    async def handle_bos_rebind_thread(
        self, channel_id: str, user_id: str,
        ticket: str | None = None, interaction: Any = None,
    ) -> None:
        """`/bos rebind-thread [ticket]` — repoint a ticket's home thread (ghost-side).

        A pure ghost-side pointer move (registry channel + binding) — no builder-os
        call, since ghost never writes runtime-state (§5.5). Recovers a ticket
        whose Discord thread was lost/archived by binding it to the current one.
        """
        actor = await self._bos_actor_or_refuse(channel_id, user_id, interaction)
        if actor is None:
            return
        ...
        if not uid:
            await self._bos_refuse(
                channel_id, interaction, "❓ No ticket",
                "Pass `ticket:<uid>` (or run in the ticket's current thread).")
            return
        rec = self.builder_registry.get(uid)
        if rec is None:
            await self._bos_refuse(
                channel_id, interaction, "❓ Unknown ticket",
                f"**{uid}** is not in the registry — nothing to rebind.")
            return
        # Repoint the registry channel (ghost-side pointer; paths pass through
        # unchanged since they are already absolute).
        await self.builder_registry.register(
            uid,
            runtime_dir=rec.runtime_dir,
            event_log=rec.event_log,
            channel_id=channel_id,
            driver_session_id=rec.driver_session_id,
            capability_token=rec.capability_token,
            assistant_channel_id=rec.assistant_channel_id,
        )
        binding = self._binding_for_ticket(uid)
        if binding is not None and binding.channel_id != channel_id:
            await self.session_mgr.bind(
                platform="discord",
                channel_id=channel_id,
                ...
                coding_cli=binding.coding_cli,
                cli_session_id=binding.cli_session_id,
                builder_ticket_uid=uid,
            )
            await self.session_mgr.unbind(binding.channel_id)
            self.monitor.start_polling(channel_id, binding.window_id)
        await self._bos_ack(
            channel_id, interaction,
            f"🧵 `/bos rebind-thread` — **{uid}** now points at this channel.")

    # -- /bos helpers -------------------------------------------------------

    async def _bos_actor_or_refuse(
        self, channel_id: str, user_id: str, interaction: Any,
    ) -> str | None:
        """Fail-closed actor gate shared by every /bos verb (AC4).

        Returns the resolved human-builder id, or ``None`` after posting a
        refusal card. The gate runs before any subprocess or state write, so an
        unmapped id (or an unconfigured builder-os) can never have a side effect.
        """
        actor = self.builder_humans.resolve(user_id)
        if actor is None:
            await self._bos_refuse(
                channel_id, interaction, "🚫 Unmapped identity — refused",
                "Your Discord account is not mapped to a builder identity, so "
                "this `/bos` command was **not** executed (fail-closed). Ask the "
                "operator to add you to `builder_humans.json`.")
            return None
        if not self.builder_launcher.configured:
            await self._bos_refuse(
                channel_id, interaction, "⚙️ builder-os not configured",
                "This ghost has no `BUILDER_OS_ROOT` configured, so `/bos` is "
                "inert here (activation is operator-scheduled).")
            return None
        return actor

    def _bos_ticket_for_channel(self, channel_id: str) -> str | None:
        """The builder ticket uid bound to *channel_id* (or None)."""
        binding = self.session_mgr.get_binding(channel_id)
        return getattr(binding, "builder_ticket_uid", None) if binding else None

    def _bos_launch_command(self, spec: LaunchSpec) -> str:
        """Resolve the base CLI binary for the spec and assemble the command."""
        try:
            base = self.launcher.resolve_cli(spec.cli).cmd
        except Exception:
            base = spec.cli
        return BuilderLauncher.build_command(spec, base)

    @staticmethod
    def _bos_thread_title(spec: LaunchSpec, issue: int) -> str:
        label = spec.display_id or spec.ticket_uid
        return f"{label} — issue #{issue}"[:100]

    @staticmethod
    def _bos_window_name(uid: str) -> str:
        """A tmux-safe window name derived from the ticket uid."""
        safe = uid.replace(":", "-").replace("/", "-").replace(" ", "-")
        return f"bos-{safe}"

    @staticmethod
    def _format_bos_status(uid: str, st: dict) -> str:
        if not st.get("admitted", True) and st.get("state") is None:
            legal = ", ".join(st.get("legal_verbs") or ["admit"])
            return f"**{uid}** — not admitted. Legal: `{legal}`"
        verbs = ", ".join(st.get("legal_verbs") or []) or "—"
        lines = [
            f"**{uid}** — state `{st.get('state')}` (epoch {st.get('epoch')})",
            f"driver session: `{st.get('driver_session_id')}`",
        ]
        if st.get("open_decision_id"):
            lines.append(f"open decision: `{st['open_decision_id']}` — answer with `/bos respond`")
        if st.get("candidate_ref"):
            lines.append(f"candidate: `{st['candidate_ref']}` (round {st.get('round')})")
        if st.get("review_verdict"):
            lines.append(f"review verdict: `{st['review_verdict']}`")
        lines.append(f"legal verbs: {verbs}")
        return "\n".join(lines)

    def _spawn_session_capture(
        self, uid: str, channel_id: str, window_id: str,
    ) -> None:
        """Arm G5 session capture without blocking the command response.

        Waits (≤5s) for the SessionStart hook to stamp ``binding.cli_session_id``
        (done by JsonlMonitor off the one session_map), then ``record-session``.
        A miss is logged, never fatal — the mapping self-heals on the next resume.
        """
        async def _capture() -> None:
            try:
                sid = await self._await_cli_session_id(channel_id, timeout=5.0)
                if not sid:
                    logger.info(
                        "bos session capture: no cli_session_id for %s within 5s",
                        uid)
                    return
                await self.builder_launcher.record_session(uid, sid, window_id)
                logger.info(
                    "bos session capture: recorded %s cli_session=%s", uid, sid)
            except Exception:
                logger.warning(
                    "bos session capture failed for %s", uid, exc_info=True)

        try:
            asyncio.get_running_loop().create_task(_capture())
        except RuntimeError:
            logger.debug("bos session capture: no running loop, skipped for %s", uid)

    async def _await_cli_session_id(
        self, channel_id: str, timeout: float,
    ) -> str | None:
        """Poll the binding for a cli_session_id (stamped by JsonlMonitor)."""
        ...
        while time.monotonic() < deadline:
            ...
            await asyncio.sleep(0.5)
        binding = self.session_mgr.get_binding(channel_id)
        return binding.cli_session_id if binding else None

    async def _bos_ack(
        self, channel_id: str, interaction: Any, text: str,
    ) -> None:
        """Deliver a plain /bos result line (interaction followup, else channel)."""
        if interaction is not None:
            await self._reply(interaction, text)
        elif self._adapter is not None:
            await self._adapter.send_message(channel_id, OutgoingMessage(text=text))

    async def _bos_refuse(
        self, channel_id: str, interaction: Any, title: str, description: str,
    ) -> None:
        """Post a /bos refusal card to the channel + clear the interaction."""
        if self._adapter is not None:
            await self._adapter.send_message(
                channel_id,
                OutgoingMessage(embed=Embed(
                    title=title, description=description, color=0xE74C3C)))
        if interaction is not None:
            await self._reply(interaction, title)
